...
func DBSize() *cobra.Command {
	dbSizeCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			_, err = fmt.Fprintf(cmd.OutOrStdout(), "State size without history = %.3f MB \n", float64(stateSizeWithoutHistory)/(1<<20))
			if err != nil {
				return err
			}

			_, err = fmt.Fprintf(cmd.OutOrStdout(), "State size with history = %.3f MB \n", float64(stateSizeWithHistory)/(1<<20))
			if err != nil {
				return err
			}

			_, err = fmt.Fprintf(cmd.OutOrStdout(), "Total DB size = %.3f MB \n", float64(totalSize)/(1<<20))
			if err != nil {
				return err
			}
